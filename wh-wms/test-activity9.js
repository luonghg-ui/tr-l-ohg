...
(async () => {
    const q1 = encodeURIComponent(JSON.stringify({target: "MEDX.GHKGEQ1P"}));
    const q2 = encodeURIComponent(JSON.stringify({targetID: 109, target: "MEDX.GHKGEQ1P"}));
    const q3 = encodeURIComponent(JSON.stringify({"target":"MEDX.GHKGEQ1P","targetID":109}));
    const urls = [
        `https://internal.thuocsi.vn/backend/core/activity/v1/list?q=${q1}`,
        `https://internal.thuocsi.vn/backend/core/activity/v1/activity/list?q=${q1}`,
        `https://internal.thuocsi.vn/backend/core/activity/v1/list?q=${q2}`,
        `https://internal.thuocsi.vn/backend/core/activity/v1/activity/list?q=${q2}`
    ];
    for(let u of urls) {
